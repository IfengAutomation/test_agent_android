...
//		clicker.clickOnScreen(view);
	...
	*/
	...
	/////////////////////////////////////////////////////////////////////
	//                                                                 //
	//     Additional method by Ifeng automation test team             //
	//                                                                 //
	/////////////////////////////////////////////////////////////////////
	 ...
	 * Drag in view rect.
	 * @param view
	 * @param startX start position on x, input percent value. e.g. 40 --> 40%
	 * @param startY start position on y, input percent value. e.g. 40 --> 40%
	 * @param endX end position on x, input percent value. e.g. 40 --> 40%
	 * @param endY end position on y, input percent value. e.g. 40 --> 40%
     * @param stepCount
     */
	public void dragInView(View view, int startX, int startY, int endX, int endY, int stepCount){
		int[] location = new int[2];
		view.getLocationOnScreen(location);
		float realStartX = view.getWidth()*startX/100f + location[0];
		float realStartY = view.getHeight()*startY/100f + location[1];
		float realEndX = view.getWidth()*endX/100f + location[0];
		float realEndY = view.getHeight()*endY/100f + location[1];
		drag(realStartX, realEndX, realStartY, realEndY, stepCount);
	}
     ...
     * @return
     */
	public TextView getTextFromParent(View parent, String text, int index){
		ArrayList<View> views = viewFetcher.getViews(parent, false);
		ArrayList<TextView> textViews = RobotiumUtils.filterViews(TextView.class, views);
		List<TextView> filteredViews = RobotiumUtils.filterViewsByText(textViews, text);
		return filteredViews.get(index);
	}
     ...
     * @return
     */
	public View getViewFromParent(View parent, String id) {
		ArrayList<View> views = viewFetcher.getViews(parent, false);
		...
		org.junit.Assert.fail("Get view from parent failed. not found view by id "+id);
		return null;
	}
	 ...
	 * @return
     */
	public View getDisplayedViews(String id){
		View[] windowDecorViews = viewFetcher.getWindowDecorViews();
		if(windowDecorViews.length == 0){
			org.junit.Assert.fail("Get displayed views failed. Not found any window decor views.");
		}

		int windowWidth = windowDecorViews[0].getWidth();
		int windowHeight = windowDecorViews[0].getHeight();
		Rect rect = new Rect(0, 0, windowWidth, windowHeight);
		Context targetContext = instrumentation.getTargetContext();
		String packageName = targetContext.getPackageName();
		int viewId = targetContext.getResources().getIdentifier(id, "id", packageName);
		ArrayList<View> allViews = viewFetcher.getAllViews(false);
		for(View view: allViews){
			Integer idOfView = Integer.valueOf(view.getId());

			if (idOfView.equals(viewId) && view.getLocalVisibleRect(rect)) {
				//check if view in window area
				return view;
			}
		}
		org.junit.Assert.fail("Get displayed view by id failed. View not found.");
		return null;
	}
